...
export default function ParsedText({
  ...
  // ใช้ undefined และประกาศชนิดไว้ชัดเจน
  let firstYT: YTInfo | undefined;
  ...
  lines.forEach((line, li) => {
    ...
    words.forEach((w, wi) => {
      ...
      if (hasHttp) {
        const yt = parseYouTube(w);
        if (yt && !firstYT) firstYT = yt; // กำหนดเป็น YTInfo ชัดเจน
        para.push(
          <a
            key={`u${li}-${wi}`}
            href={w}
  ...
  if (firstYT != null) {
    // สร้างตัวแปรท้องถิ่นให้ TS แคบชนิดเหลือ YTInfo
    const yt: YTInfo = firstYT;
    nodes.push(
      <div key={`yt-${yt.id}`} className="mt-2 rounded-xl overflow-hidden border border-gray-200">
